...
class Rocket(implicit p: Parameters) extends CoreModule()(p)
  ...
  val rfdfi=Reg(init = Vec.fill(31){Bits(0,width = xLen)})//flang
  ...
  //flang
  io.rocc.canexception := io.rocc.coreexception && id_ctrl.mem && id_ctrl.mem_cmd === M_XRD && !id_ctrl.fp && !ctrl_killd//csr.io.decode(0).rocc_illegal// !ctrl_killd
  ...
  val id_illegal_insn = !id_ctrl.legal ||
    ...
    //flang
    io.rocc.coreexception && id_ctrl.mem && id_ctrl.mem_cmd === M_XRD && !id_ctrl.fp ||
  ...
  //wxrqw
  val reg_wxr = Reg(init = Bits(0,width=64))

  io.rocc.reg_ready := Bool(true)
  when(io.rocc.reg_valid ){
    reg_wxr :=io.rocc.reg_value
  }
  //wxrqw

 // flang
  when (rf_wen){
	  rfdfi(rf_waddr) := rf_wdata
  }
  ...
  val ctrl_stalld =
    io.rocc.corestall || //flang
    io.rocc.corestall_wxr ||  //wxrqw
  ...
  //wxrqw
  //** the "mem siganl need 1 cycyle latency"
  val cfi_v = Reg(init = Bool(false))
  val cfi_taken = Reg(init = Bool(false))
  val cfi_isCFI = Reg(init = Bool(false))
  val cfi_Type = Reg(init = Bits(0,width=3))
  val cfi_pc = Reg(init = Bits(0,width=40))
  val cfi_miss = Reg(init = Bool(false))

  cfi_v:= mem_reg_valid && !take_pc_wb 
  cfi_taken:= mem_cfi_taken
  cfi_isCFI:= mem_cfi
  cfi_Type:= 
       Mux((mem_ctrl.jal) && mem_waddr(0), CFIType.call,   //normal call
    Mux(mem_ctrl.jalr && mem_reg_inst(19,15) === BitPat("b00?01"), CFIType.ret, //mormal ret
    Mux(mem_ctrl.jalr && mem_waddr(0), 4.U,    //jarl call
    Mux(mem_ctrl.jalr, CFIType.jump,           //normal long jump
    CFIType.branch))))
  cfi_pc :=  mem_reg_pc //io.imem.btb_update.bits.pc
  cfi_miss := mem_wrong_npc



  io.rocc.cfi.bits.valid := RegNext(cfi_v)
  io.rocc.cfi.bits.taken := RegNext(cfi_taken)
  io.rocc.cfi.bits.isCFI := RegNext(cfi_isCFI)
  io.rocc.cfi.bits.cfiType := RegNext(cfi_Type)
  io.rocc.cfi.bits.pc := RegNext(cfi_pc)
  io.rocc.cfi.bits.mispredict := RegNext(cfi_miss)
  io.rocc.cfi.bits.isSP := rf.access(2)
...
//flang++++++++++++++
  /*
  val addrbuf = Reg(init = Vec.fill(64){Bits(0,width = xLen)})
  val addrbuf_p = Reg(init = Bits(0,width = xLen))
  when(io.dmem.req.valid && io.dmem.req.ready){
		addrbuf(addrbuf_p(5,0)) := io.dmem.req.bits.addr
		addrbuf_p := addrbuf_p + Bits(1)
	}
	when(wb_reg_valid && wb_ctrl.rocc && !replay_wb_common){
		printf("++++++++++++++++++\n")
		printf("buf_p %d\n", addrbuf_p)
		for (i<- 0 to 63)
		printf("buf %d value: %x\n", UInt(i), addrbuf(UInt(i)))
	}
  */
  
	io.rocc.corestalled := ctrl_stalld//pass the stalling status
  
	val dfi_cycle = Reg(init = Bits(0,width = xLen))
	dfi_cycle := dfi_cycle + Bits(1)

	val dfi_rocc_valid_buf = Reg(init = Vec.fill(32){Bits(0,width = xLen)})//for adding delay
	for (i<- 0 to 30)
	dfi_rocc_valid_buf(31-i) := dfi_rocc_valid_buf(31-i-1)
	dfi_rocc_valid_buf(0) := (wb_reg_valid<<2) | (wb_ctrl.rocc<<1) | (!replay_wb_common)
	io.rocc.cmdvalid := io.rocc.cmd.valid
	io.rocc.dfi.bits.cycle_now := dfi_cycle
	/*
	when(io.rocc.cmd.valid){
		printf("--------cmd valid now!!!!!!!, at %x\n",dfi_cycle)
		printf("rocc ready? %x\n",io.rocc.cmd.ready)
		for (i<- 0 to 10)
		printf("rocc valid status, %x cycle: %x\n",dfi_rocc_valid_buf(UInt(i)) ,dfi_cycle-Bits(i))
	}
	*/
	//function info
	val dfi_jaltarget_buf = Reg(init = Vec.fill(8){SInt(0,width = xLen)})//for adding delay
	val dfi_jalrtarget_buf = Reg(init = Vec.fill(8){SInt(0,width = xLen)})//for adding delay
	val dfi_isjalr_buf = Reg(init = Vec.fill(8){Bool(false)})//for adding delay
	val dfi_callvalid_buf = Reg(init = Vec.fill(8){Bool(false)})//for adding delay
	val dfi_retvalid_buf = Reg(init = Vec.fill(8){Bool(false)})//for adding delay
	val dfi_funcretaddr_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcstackaddr_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg0_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg1_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg2_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg3_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg4_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	val dfi_funcarg5_buf = Reg(init = Vec.fill(8){UInt(0,width = xLen)})//for adding delay
	
	val dfi_callvalid = (mem_ctrl.jal || mem_ctrl.jalr) && mem_waddr(0)
	val dfi_retvalid = mem_ctrl.jalr && mem_reg_inst(19,15) === BitPat("b00?01")

	when(!ctrl_stalld){
	for (i<- 0 to 6)
	dfi_jaltarget_buf(7-i) := dfi_jaltarget_buf(7-i-1)
	dfi_jaltarget_buf(0) := mem_br_target
	
	for (i<- 0 to 6)
	dfi_jalrtarget_buf(7-i) := dfi_jalrtarget_buf(7-i-1)
	dfi_jalrtarget_buf(0) := ex_rs(0).asSInt
	
	for (i<- 0 to 6)
	dfi_isjalr_buf(7-i) := dfi_isjalr_buf(7-i-1)
	dfi_isjalr_buf(0) := mem_ctrl.jalr
	
	for (i<- 0 to 6)
	dfi_callvalid_buf(7-i) := dfi_callvalid_buf(7-i-1)
	dfi_callvalid_buf(0) := dfi_callvalid
	
	for (i<- 0 to 6)
	dfi_retvalid_buf(7-i) := dfi_retvalid_buf(7-i-1)
	dfi_retvalid_buf(0) := dfi_retvalid
	
	for (i<- 0 to 6)
	dfi_funcretaddr_buf(7-i) := dfi_funcretaddr_buf(7-i-1)
	dfi_funcretaddr_buf(0) := rfdfi(1)
	
	for (i<- 0 to 6)
	dfi_funcstackaddr_buf(7-i) := dfi_funcstackaddr_buf(7-i-1)
	dfi_funcstackaddr_buf(0) := rfdfi(2)
	
	for (i<- 0 to 6)
	dfi_funcarg0_buf(7-i) := dfi_funcarg0_buf(7-i-1)
	dfi_funcarg0_buf(0) := rfdfi(10)
	
	for (i<- 0 to 6)
	dfi_funcarg1_buf(7-i) := dfi_funcarg1_buf(7-i-1)
	dfi_funcarg1_buf(0) := rfdfi(11)
	
	for (i<- 0 to 6)
	dfi_funcarg2_buf(7-i) := dfi_funcarg2_buf(7-i-1)
	dfi_funcarg2_buf(0) := rfdfi(12)
	
	for (i<- 0 to 6)
	dfi_funcarg3_buf(7-i) := dfi_funcarg3_buf(7-i-1)
	dfi_funcarg3_buf(0) := rfdfi(13)
	
	for (i<- 0 to 6)
	dfi_funcarg4_buf(7-i) := dfi_funcarg4_buf(7-i-1)
	dfi_funcarg4_buf(0) := rfdfi(14)
	
	for (i<- 0 to 6)
	dfi_funcarg5_buf(7-i) := dfi_funcarg5_buf(7-i-1)
	dfi_funcarg5_buf(0) := rfdfi(15)
	}
	
	//io.rocc.dfi.bits.brtarget := Mux(dfi_isjalr_buf,ex_rs(0).asSInt,mem_br_target)
	//io.rocc.dfi.bits.callvalid := dfi_callvalid
	//io.rocc.dfi.bits.retvalid := dfi_retvalid
	io.rocc.dfi.bits.funcretaddr := rfdfi(1)
	io.rocc.dfi.bits.funcstackaddr := rfdfi(2)
	//io.rocc.dfi.bits.funcarg0 := rf.access(10)
	//io.rocc.dfi.bits.funcarg1 := rf.access(11)
	//io.rocc.dfi.bits.funcarg2 := rf.access(12)
	
	//io.rocc.dfi.bits.brtarget := Mux(dfi_isjalr_buf(0),dfi_jalrtarget_buf(1),dfi_jaltarget_buf(0))//dfi_brtarget_buf(1)
	//io.rocc.dfi.bits.callvalid := dfi_callvalid_buf(0)
	//io.rocc.dfi.bits.retvalid := dfi_retvalid_buf(0)
	//io.rocc.dfi.bits.funcretaddr := dfi_funcretaddr_buf(0)
	//io.rocc.dfi.bits.funcstackaddr := dfi_funcstackaddr_buf(0)
	io.rocc.dfi.bits.funcarg0 := rfdfi(10)
	io.rocc.dfi.bits.funcarg1 := rfdfi(11)
	io.rocc.dfi.bits.funcarg2 := rfdfi(12)
	
	
	io.rocc.dfi.bits.brtarget := Mux(dfi_isjalr_buf(1),dfi_jalrtarget_buf(2),dfi_jaltarget_buf(1))//dfi_brtarget_buf(1)
	io.rocc.dfi.bits.callvalid := dfi_callvalid_buf(1)
	io.rocc.dfi.bits.retvalid := dfi_retvalid_buf(1)
	//io.rocc.dfi.bits.funcretaddr := dfi_funcretaddr_buf(0)
	//io.rocc.dfi.bits.funcstackaddr := dfi_funcstackaddr_buf(0)
	//io.rocc.dfi.bits.funcarg0 := dfi_funcarg0_buf(1)
	//io.rocc.dfi.bits.funcarg1 := dfi_funcarg1_buf(1)
	//io.rocc.dfi.bits.funcarg2 := dfi_funcarg2_buf(1)
	//io.rocc.dfi.bits.funcarg3 := dfi_funcarg3_buf(1)
	//io.rocc.dfi.bits.funcarg4 := dfi_funcarg4_buf(1)
	//io.rocc.dfi.bits.funcarg5 := dfi_funcarg5_buf(1)
	
	//thread
	io.rocc.dfi.bits.threadptr := rfdfi(4)
	
	//target address
	val dfi_cycle_buf = Reg(init = Vec.fill(8){Bits(0,width = xLen)})//for adding delay
	val dfi_addr_buf = Reg(init = Vec.fill(8){Bits(0,width = xLen)})//for adding delay
	val dfi_valid_buf = Reg(init = Vec.fill(8){Bool(false)})//for adding delay
	val dfi_cmd_buf = Reg(init = Vec.fill(8){Bits(0,width = xLen)})//for adding delay
  
	for (i<- 0 to 6)
	dfi_valid_buf(7-i) := dfi_valid_buf(7-i-1)
	dfi_valid_buf(0) := io.dmem.req.valid && io.dmem.req.ready//wb_valid

	for (i<- 0 to 6)
	dfi_addr_buf(7-i) := dfi_addr_buf(7-i-1)
	dfi_addr_buf(0) := io.dmem.req.bits.addr

	for (i<- 0 to 6)
	dfi_cmd_buf(7-i) := dfi_cmd_buf(7-i-1)
	dfi_cmd_buf(0) := io.dmem.req.bits.cmd
	
	for (i<- 0 to 6)
	dfi_cycle_buf(7-i) := dfi_cycle_buf(7-i-1)
	dfi_cycle_buf(0) := dfi_cycle

	io.rocc.test := Bits(0x2121)
	io.rocc.dfi.bits.priv := csr.io.status.prv

	io.rocc.dfi.bits.valid := dfi_valid_buf(1)
	io.rocc.dfi.bits.taraddr := dfi_addr_buf(1)
	io.rocc.dfi.bits.cmd := dfi_cmd_buf(1)
	io.rocc.dfi.bits.cycle := dfi_cycle_buf(1)
	/*
	io.rocc.dfi.bits.valid := io.dmem.req.valid && io.dmem.req.ready
	io.rocc.dfi.bits.taraddr := io.dmem.req.bits.addr
	io.rocc.dfi.bits.cmd := io.dmem.req.bits.cmd
	io.rocc.dfi.bits.cycle := dfi_cycle
	*/
	io.rocc.dfi.bits.svalid := dfi_valid_buf(1) && (dfi_cmd_buf(1) === M_XWR)
	io.rocc.dfi.bits.staraddr := dfi_addr_buf(2)
	io.rocc.dfi.bits.lvalid := dfi_valid_buf(3) && (dfi_cmd_buf(3) === M_XRD)
	io.rocc.dfi.bits.ltaraddr := dfi_addr_buf(2)

	/*
	when(io.dmem.req.valid && io.dmem.req.ready && io.dmem.req.bits.addr > Bits(0x80000000L) && io.dmem.req.bits.addr < Bits(0x80022800L) ){
	  when(io.dmem.req.bits.cmd === M_XWR){
		printf("==write addr: %x\n",io.dmem.req.bits.addr)
	}.elsewhen(io.dmem.req.bits.cmd === M_XRD){
		printf("==read addr: %x\n",io.dmem.req.bits.addr)
	}
	}*/

	//io.rocc.dfi.valid := io.dmem.req.valid && io.dmem.req.ready && io.rocc.cmd.ready
	//io.rocc.dfi.bits.taraddr := io.dmem.req.bits.addr
	//io.rocc.dfi.bits.cmd := io.dmem.req.bits.cmd

	io.rocc.dfi.bits.test := Bits(0x1123)
//flang--------------------------
...
class RegFile(n: Int, w: Int, zero: Boolean = false) {
  ...
  def access(addr: UInt) = rf(~addr(log2Up(n)-1,0))//flang
