...
0xDEADBEEF,0xDEADBEEF,0xDEADBEEF,0xDEADBEEF, 0xDEADBEEF,448}; //448 => 0700 mode
...
static int rop_sled[7] = {0,0,0,0,0,0,0};
...
void mylongjmp(int dummy, void* pointer, int data){
	longjmp(pointer,data);
}
...
void perform_attack(FILE *output_stream,
 ...
 // char stack_buffer2[1];
  ...
  printf("buffer1 addr %lx buffer2 addr %lx tar addr %lx ph addr %lx\n",(unsigned long)bss_buffer1, (unsigned long)bss_buffer2,(unsigned long)&bss_mem_ptr, (unsigned long)placeholder);
  ...
  case INDIRECT:
    ...
    case HEAP:
      ...
      break;
    case BSS:
      target_addr = &bss_mem_ptr[31];
      bss_mem_ptr[31] = &bss_dummy_value;
      break;
    case DATA:
  ...
  case MEMCPY:
    ...
    memcpy(buffer, payload.buffer, payload.size );
    break;
  case STRCPY:
...
boolean build_payload(CHARPAYLOAD *payload) {
  ...
  //flang
  if(!(attack.function == NOATTACK) && !(attack.function == MEMCPY) && !(attack.function == HOMEBREW)) {
