...
class WithRoccExample extends Config((site, here, up) => {
  case BuildRoCC => List(
    ...
    (p: Parameters) => {
        val accumulator = LazyModule(new Co_processor(OpcodeSet.custom0, n = 4)(p))
        accumulator
    },
    /*
    (p: Parameters) => {
    ...
    /*
    (p: Parameters) => {
        val dficheck = LazyModule(new DFIcheck(OpcodeSet.custom0)(p))
        dficheck
    }
    */
    /*,
    (p: Parameters) => {
        val dfitest = LazyModule(new DFItest(OpcodeSet.custom1)(p))
        dfitest
    }*/
    )
})
