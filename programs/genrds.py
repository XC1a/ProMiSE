...
sfunc={'dfi_rocc_stlddebug':'','init_ori_dfi':'','dfi_rocc_debug':'','dfi_new_rocc_debug':'','cfi_rocc_debug':'','none_sec_init':'','init_roccinstr_dfi':'','cfi_init':'','dfi_new_init':'', 'dfi_func_signal':'','init_soft_dfi':'','init_noinstrument_dfi':'','dfi_delay':'','execute_dficheck':'','init_2core_noic_dfi':'','dfi_send_sl':'','dfi_send_lib':'','init_2core_dfi':'','init_pim_dfi':'','init_lite_pim_dfi':'','init_lite_hbm_dfi':'','init_soft_pim_dfi':'','init_soft_hbm_dfi':'','pim_dfi_timestamp':'','pim_dfi_end_timestamp':'','dfi_debug':'','pim_dfi_func_signal':'','pim_dfi_print':''}
...
def read_usr_instr(filename):
	text = read_file(filename)
	for id in text:
		tmp = str.split(id)
		usr_instr[int(tmp[0])] = int(tmp[1])
	print(usr_instr)
...
def classify_by_func(inst,idinst):
	#this function will transform the original list of instruction(instruction with ids) to the
	#format that each instruction is classified in its function.
	func_inst=[]
	last_func=''
	for it in inst:
		if last_func!=it['func']:
			last_func=it['func']
			func_inst.append({'func':last_func,'inst':[]})
		func_inst[-1]['inst'].append(it)
	func_idinst=[]
	last_func=''
	for ii in idinst:
		if last_func!=ii['func']:
			last_func=ii['func']
			func_idinst.append({'func':last_func,'inst':[]})
		func_idinst[-1]['inst'].append(ii)
	
	delete=[]
	i=0
	while i<len(func_inst):
		found=0
		for fii in func_idinst:
			if func_inst[i]['func']==fii['func']:
				func_inst[i]['idinst']=fii['inst']
				found=1
				break
		if found==0:
			delete.append(i)
...
def split_by_branch(inst):
	i=0
	while i<len(inst):
		temp=[[]]
		for it in inst[i]['inst']:
			temp[-1].append(it)
			if it['type'][0]=='b':
				temp.append([])
		if temp[-1]==[]:
...
def match_sl(inst):
	"""
	for it in inst:
		if len(it['inst'])==len(it['idinst']):
			print "Block # match. Function",it['func'],"Block #",len(it['inst'])
		else:
			print "Block # NOT match. Function",it['func'],"Inst Block #",len(it['inst']),"Inst with id block #",len(it['idinst'])
	"""
	i=0
	for ins in inst:
		inst_temp=ins['inst']
		idinst_temp=ins['idinst']
		delete=[]
		j=0
		for it in inst_temp:
			if it['type']=='l':
				if len(re.findall('pc',it['reg'][1]))>0:
					delete.append(j)
			j+=1
		delete.reverse()
		for d in delete:
		...
		for it in inst_temp:
			if it['type']=='l':
				count_load_inst+=1
		for it in idinst_temp:
			if it['type']=='l':
				count_load_idinst+=1
		count_store_inst=0
		count_store_idinst=0
		for it in inst_temp:
			if it['type']=='s':
				count_store_inst+=1
		for it in idinst_temp:
			if it['type']=='s':
				count_store_idinst+=1
		"""
		if count_load_inst==count_load_idinst and count_store_inst==count_store_idinst:
			print "Match",ins['func'],count_store_inst,count_load_inst
		else:
			print "NOT Match",ins['func'],count_store_inst,count_store_idinst,"|||",count_load_inst,count_load_idinst
		"""
		ii=0
		iid=0
		while ii<len(inst_temp) and iid<len(idinst_temp):
			if idinst_temp[iid]['type']=='l':
				if inst_temp[ii]['type']=='l':
					inst[i]['idinst'][iid]['addr']=inst_temp[ii]['addr']
					iid+=1
				ii+=1
			elif idinst_temp[iid]['type']=='s':
				if inst_temp[ii]['type']=='s':
					inst[i]['idinst'][iid]['addr']=inst_temp[ii]['addr']
					iid+=1
				ii+=1
			else:
...
def find_addr(nums,id2addr):
	indice=[-1 for i in range(len(nums))]
	i_n=0
	i_i=0
	while i_n<len(nums) and i_i<len(id2addr):
		if nums[i_n]<id2addr[i_i][0]:
			i_n+=1
		elif nums[i_n]>id2addr[i_i][0]:
			i_i+=1
		else:
			indice[i_n]=id2addr[i_i][1]
			i_n+=1
			i_i+=1
	return indice
...
def trans_rds(inst):
	...
	for it in inst:
		for ii in it['idinst']:
			if ii['type']=='s':
				id2addr.append([ii['id'],ii['addr']])
			elif ii['type']=='l':
				#id2addr.append([ii['id'],ii['addr']])
				rds.append([ii['addr'],[],ii['rds']])
	
	i=0
	while i<len(rds):
		print '{0}/{1}\r'.format(i,len(rds)-1),
		#print i,'/',len(rds)-1
		indice=find_addr(rds[i][2],id2addr)
		i_i=0
		while i_i<len(indice):
			if indice[i_i]==-1:
				indice[i_i]='N_list'
			elif len(indice[i_i])==0:
				indice[i_i]='N_match'
			i_i+=1
		rds[i][1]=indice
		del rds[i][2]
...
def inst_debug(text):
	...
	line_inst=[]
	while i<len(text):
		if timestampfunc=='main':
			exp='define .* @'+timestampfunc+'\(.*\) .* \{'
		else:
...
def cfi_inst_rocc_debug(text):
	...
	line_inst.reverse()
	for l in line_inst:
		text.insert(l,'call void @'+sfunc['cfi_rocc_debug']+'()\n')

def dfi_inst_rocc_debug(text):
	i=0
	state=0
	line_inst=[]
	while i<len(text):
	...
	line_inst.reverse()
	for l in line_inst:
		text.insert(l,'call void @'+sfunc['dfi_new_rocc_debug']+'()\n')
...
def inst_init(fake,base_id):
	...
	if exp_mode=="ori":
		...
		#string.append('call void @'+sfunc['dfi_func_signal']+'()\n')
		#bit 21: is ret
		string.append('%'+str(base_id)+' = call i8* @llvm.frameaddress(i32 0)\n')
		string.append('%'+str(base_id+1)+' = ptrtoint i8* %'+str(base_id)+' to i32\n')
		string.append('store i32 '+str((1<<21)|ret_id)+', i32* @dfi_reg_signal, align 4\n')
		string.append('store i32 %'+str(base_id+1)+', i32* @dfi_reg_signal, align 4\n')
	elif exp_mode=="soft":
		string.append('call void asm sideeffect ".word '+hex(0xc800000b)+'", ""()\n')
		#string.append('call void @'+sfunc['dfi_func_signal']+'()\n')
		...
		string.append('%'+str(base_id+1)+' = call i8* @llvm.frameaddress(i32 0)\n')
		string.append('%'+str(base_id+2)+' = ptrtoint i8* %'+str(base_id+1)+' to i32\n')
		string.append('%'+str(base_id+3)+' = load i32*, i32** @dfi_reg_t, align 4\n')
		
		string.append('store i32 33440000, i32* %'+str(base_id+3)+', align 4\n')#
		#string.append('store i32 33440001, i32* %'+str(base_id+3)+', align 4\n')#
		string.append('%'+str(base_id+4)+' = icmp eq i32 %'+str(base_id+2)+', 4294967295\n')
		...
		#string.append('store i32 33440002, i32* %'+str(base_id+3)+', align 4\n')#
		string.append('%'+str(base_id+7)+' = mul nsw i32 32, %'+str(base_id+2)+'\n')
		...
		#string.append('store i32 33440003, i32* %'+str(base_id+3)+', align 4\n')#
		string.append('store i'+str(bf_len)+' '+str(ret_id)+', i'+str(bf_len)+'* %'+str(base_id+10)+', align '+str(bf_len/8)+'\n')
		#string.append('store i32 33440004, i32* %'+str(base_id+3)+', align 4\n')#
		string.append('br label %'+str(base_id+11)+'\n')
		string.append('; <label>:'+str(base_id+11)+':\n')
		string.append('store i32 33440005, i32* %'+str(base_id+3)+', align 4\n')#
	return string
...
def inst_sl_old_cycletest(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos):
	#added the memcpt etc. functions
	#changed l/r shift bits
	load_from=copy.deepcopy(load_from)
	if fake:#only return the instrumentation length
	...
	if sl=='s':
		if pos==0:
			return []
		string=[]
		string.append('store i32 33440000, i32* %'+str(init_base_id+3)+', align 4\n')#
		#string.append('store i32 33440001, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('%'+str(base_id)+' = ptrtoint '+load_from+' to i32\n')
		string.append('%'+str(base_id+1)+' = icmp eq i32 %'+str(base_id)+', 4294967295\n')
		string.append('br i1 %'+str(base_id+1)+', label %'+str(base_id+2)+', label %'+str(base_id+3)+'\n')
		string.append('; <label>:'+str(base_id+2)+':\n')
		string.append('br label %'+str(base_id+8)+'\n')
		string.append('; <label>:'+str(base_id+3)+':\n')
		#string.append('store i32 33440002, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('%'+str(base_id+4)+' = mul nsw i32 32, %'+str(base_id)+'\n')
		string.append('%'+str(base_id+5)+' = lshr i32 %'+str(base_id+4)+', 7\n')
		string.append('%'+str(base_id+6)+' = add nsw i32 10, %'+str(base_id+5)+'\n')
		string.append('%'+str(base_id+7)+' = getelementptr inbounds i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(init_base_id)+', i32 %'+str(base_id+6)+'\n')
		#string.append('store i32 33440003, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('store i'+str(bf_len)+' '+str(node_id)+', i'+str(bf_len)+'* %'+str(base_id+7)+', align '+str(bf_len/8)+'\n')
		#string.append('store i32 33440004, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('br label %'+str(base_id+8)+'\n')
		string.append('; <label>:'+str(base_id+8)+':\n')
		string.append('store i32 33440005, i32* %'+str(init_base_id+3)+', align 4\n')#
	elif sl=='l':
		if pos==0:
			return []
		string=[]
		string.append('store i32 33441000, i32* %'+str(init_base_id+3)+', align 4\n')#
		#string.append('store i32 33441001, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('%'+str(base_id)+' = ptrtoint '+load_from+' to i32\n')
		string.append('%'+str(base_id+1)+' = mul nsw i32 32, %'+str(base_id)+'\n')
		string.append('%'+str(base_id+2)+' = lshr i32 %'+str(base_id+1)+', 7\n')
		string.append('%'+str(base_id+3)+' = add nsw i32 10, %'+str(base_id+2)+'\n')
		string.append('%'+str(base_id+4)+' = getelementptr inbounds i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(init_base_id)+', i32 %'+str(base_id+3)+'\n')
		#string.append('store i32 33441002, i32* %'+str(init_base_id+3)+', align 4\n')#
		string.append('%'+str(base_id+5)+' = load i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(base_id+4)+', align '+str(bf_len/8)+'\n')
		#string.append('store i32 33441003, i32* %'+str(init_base_id+3)+', align 4\n')#
		now_id=base_id+5
		parts=inst_sl_old_find_cont(rds)
		final_id=now_id
		if len(parts)==0:
		...
		string.append('br label %'+str(final_id+2)+'\n')
		string.append('; <label>:'+str(final_id+2)+':\n')
		string.append('store i32 33441004, i32* %'+str(init_base_id+3)+', align 4\n')#
	elif len(sl)>=3 and sl[0:2]=='bc':
		lib_p=int(sl[2:])
		lib_s=sp_inst_funcs[lib_p]['s']
		lib_l=sp_inst_funcs[lib_p]['l']
		...
		if source!=[]:
			...
			for p in parts:
				if len(p)<=2:
					...
					final_id+=4
			string.append('store i32 33442000, i32* %'+str(init_base_id+3)+', align 4\n')#
			#string.append('store i32 33442001, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(base_id)+' = alloca i'+str(rlen)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(base_id+1)+' = alloca i32, align 4\n')
			string.append('store i'+str(rlen)+' 0, i'+str(rlen)+'* %'+str(base_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(base_id+2)+' = ptrtoint '+source+' to i32\n')
			...
			string.append('br label %'+str(base_id+3)+'\n')
			string.append('; <label>:'+str(base_id+3)+':\n')
			#string.append('store i32 33442002, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(base_id+4)+' = load i'+str(rlen)+', i'+str(rlen)+'* %'+str(base_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(base_id+5)+' = icmp sge i'+str(rlen)+' %'+str(base_id+4)+', '+rfrom+'\n')
			#string.append('store i32 33442003, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('br i1 %'+str(base_id+5)+', label %'+str(final_id+7)+', label %'+str(base_id+6)+'\n')
			string.append('; <label>:'+str(base_id+6)+':\n')
			#string.append('store i32 33442004, i32* %'+str(init_base_id+3)+', align 4\n')#
			
			string.append('%'+str(base_id+7)+' = load i32, i32* %'+str(base_id+1)+', align 4\n')
			#string.append('store i32 33442005, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(base_id+8)+' = mul nsw i32 32, %'+str(base_id+7)+'\n')
			string.append('%'+str(base_id+9)+' = lshr i32 %'+str(base_id+8)+', 7\n')
			string.append('%'+str(base_id+10)+' = add nsw i32 10, %'+str(base_id+9)+'\n')
			string.append('%'+str(base_id+11)+' = getelementptr inbounds i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(init_base_id)+', i32 %'+str(base_id+10)+'\n')
			#string.append('store i32 33442006, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(base_id+12)+' = load i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(base_id+11)+', align '+str(bf_len/8)+'\n')
			#string.append('store i32 33442007, i32* %'+str(init_base_id+3)+', align 4\n')#
			now_id=base_id+12
			if len(parts)==0:
				string.append('br label %'+str(final_id+2)+'\n')
				string.append('; <label>:'+str(now_id+1)+':\n')
			...
			#string.append('store i32 33442008, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id+3)+' = load i'+str(rlen)+', i'+str(rlen)+'* %'+str(base_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(final_id+4)+' = add nsw i'+str(rlen)+' %'+str(final_id+3)+', 4\n')
			string.append('store i'+str(rlen)+' %'+str(final_id+4)+', i'+str(rlen)+'* %'+str(base_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(final_id+5)+' = load i32, i32* %'+str(base_id+1)+', align 4\n')
			string.append('%'+str(final_id+6)+' = add nsw i32 %'+str(final_id+5)+', 4\n')
			string.append('store i32 %'+str(final_id+6)+', i32* %'+str(base_id+1)+', align 4\n')
			#string.append('store i32 33442009, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('br label %'+str(base_id+3)+'\n')
			string.append('; <label>:'+str(final_id+7)+':\n')
			string.append('store i32 33442010, i32* %'+str(init_base_id+3)+', align 4\n')#
			
			final_id=final_id+8
		if target!=[]:
			string.append('store i32 33443000, i32* %'+str(init_base_id+3)+', align 4\n')#
			#string.append('store i32 33443001, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id)+' = alloca i'+str(rlen)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(final_id+1)+' = alloca i32, align 4\n')
			string.append('store i'+str(rlen)+' 0, i'+str(rlen)+'* %'+str(final_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(final_id+2)+' = ptrtoint '+target+' to i32\n')
			...
			string.append('br label %'+str(final_id+3)+'\n')
			string.append('; <label>:'+str(final_id+3)+':\n')
			#string.append('store i32 33443002, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id+4)+' = load i'+str(rlen)+', i'+str(rlen)+'* %'+str(final_id)+', align '+str(rlen/8)+'\n')
			string.append('%'+str(final_id+5)+' = icmp sge i'+str(rlen)+' %'+str(final_id+4)+', '+rfrom+'\n')
			#string.append('store i32 33443003, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('br i1 %'+str(final_id+5)+', label %'+str(final_id+20)+', label %'+str(final_id+6)+'\n')
			string.append('; <label>:'+str(final_id+6)+':\n')
			
			#string.append('store i32 33443004, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id+7)+' = load i32, i32* %'+str(final_id+1)+', align 4\n')
			#string.append('store i32 33443005, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id+8)+' = icmp eq i32 %'+str(final_id+7)+', 4294967295\n')
			string.append('br i1 %'+str(final_id+8)+', label %'+str(final_id+9)+', label %'+str(final_id+10)+'\n')
			string.append('; <label>:'+str(final_id+9)+':\n')
			string.append('br label %'+str(final_id+15)+'\n')
			string.append('; <label>:'+str(final_id+10)+':\n')
			#string.append('store i32 33443006, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('%'+str(final_id+11)+' = mul nsw i32 32, %'+str(final_id+7)+'\n')
			string.append('%'+str(final_id+12)+' = lshr i32 %'+str(final_id+11)+', 7\n')
			string.append('%'+str(final_id+13)+' = add nsw i32 10, %'+str(final_id+12)+'\n')
			string.append('%'+str(final_id+14)+' = getelementptr inbounds i'+str(bf_len)+', i'+str(bf_len)+'* %'+str(init_base_id)+', i32 %'+str(final_id+13)+'\n')
			#string.append('store i32 33443007, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('store i'+str(bf_len)+' '+str(node_id)+', i'+str(bf_len)+'* %'+str(final_id+14)+', align '+str(bf_len/8)+'\n')
			#string.append('store i32 33443008, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('br label %'+str(final_id+15)+'\n')
			string.append('; <label>:'+str(final_id+15)+':\n')
			#string.append('store i32 33443009, i32* %'+str(init_base_id+3)+', align 4\n')#
			...
			string.append('%'+str(final_id+19)+' = add nsw i32 %'+str(final_id+18)+', 4\n')
			string.append('store i32 %'+str(final_id+19)+', i32* %'+str(final_id+1)+', align 4\n')
			#string.append('store i32 33443010, i32* %'+str(init_base_id+3)+', align 4\n')#
			string.append('br label %'+str(final_id+3)+'\n')
			string.append('; <label>:'+str(final_id+20)+':\n')
			string.append('store i32 33443011, i32* %'+str(init_base_id+3)+', align 4\n')#
	elif sl=='bt':
		string=[]
		string.append('store i32 33441000, i32* %'+str(init_base_id+3)+', align 4\n')#
		#string.append('store i32 33441001, i32* %'+str(init_base_id+3)+', align 4\n')#
...
def inst_sl_cfi(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos):
...
def inst_sl_roccinstr(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos):
	...
	if sl=='s':
		if pos==0:
			...
			if hdfi_instr==1:
				if is_usr_instr and (node_id in usr_instr):
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00000000+(usr_instr[node_id]))<<7)|0x0b)+'", ""()\n')
				elif is_usr_instr == 0:
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00000000+(0x1))<<7)|0x0b)+'", ""()\n')
			elif whlist_instr==1:
				if is_usr_instr and (node_id in usr_instr):
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00000000+(usr_instr[node_id]))<<7)|0x0b)+'", ""()\n')
		...
		if pos==0:
			...
			if hdfi_instr==1:
				if is_usr_instr and (node_id in usr_instr):
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00010000+(usr_instr[node_id]))<<7)|0x0b)+'", ""()\n')
				else:
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00010000+(0x1))<<7)|0x0b)+'", ""()\n')
			elif whlist_instr==1:
				if is_usr_instr and (node_id in usr_instr):
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00010000+(usr_instr[node_id]))<<7)|0x0b)+'", ""()\n')
				elif is_usr_instr==0:
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info(0x00010000+(0x11))<<7)|0x0b)+'", ""()\n')
			else:
		...
		if hdfi_instr==1:
			if is_usr_instr and (node_id in usr_instr):
				string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(mark<<17)+(1<<16)+(isfunc<<21)+usr_instr[node_id])<<7)|0x0b)+'", ""()\n')
			else:
				string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(mark<<17)+(1<<16)+(isfunc<<21)+1)<<7)|0x0b)+'", ""()\n')
		elif whlist_instr==1:
			if is_usr_instr and (node_id in usr_instr):
				string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(mark<<17)+(1<<16)+(isfunc<<21)+usr_instr[node_id])<<7)|0x0b)+'", ""()\n')
				print string
		else:
			string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(mark<<17)+(1<<16)+(isfunc<<21)+node_id)<<7)|0x0b)+'", ""()\n')
	elif sl=='bt':
		string=[]
		if anyfunc:
			#mark=4
			if hdfi_instr==1:
				if is_usr_instr and (0xffff in usr_instr): #0xffff is a symbol of ret
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(0<<16)+(1<<21)+(usr_instr[0xffff]))<<7)|0x0b)+'", ""()\n')
				else:
					string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(0<<16)+(1<<21)+1)<<7)|0x0b)+'", ""()\n')
			elif whlist_instr==1:
				string = []
			else:
				string.append('call void asm sideeffect ".word '+hex((rocc_trans_info((1<<19)+(0<<16)+(1<<21)+ret_id)<<7)|0x0b)+'", ""()\n')
	else:
		string=[]
	return string

def inst_sl_new_5(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos):
	#this is for gem5, only instrument on store for reference
	#added the memcpy etc. functions
	load_from=copy.deepcopy(load_from)
	if fake:#only return the instrumentation length
	...
	if sl=='s':
		if pos==0:
			...
			string.append('store i32 '+str(0x00000000+node_id)+', i32* @dfi_reg_signal, align 4\n')
			if(signal_delay):
				string.append('call void @'+sfunc['dfi_delay']+'(i32 '+str(signal_delay)+')\n')
	elif sl=='l':
		if pos==0:
			return []
		else:
		...
		#bit 17: is lib, bit 18: is 64(/32), bit 19: is r, bit 20: is w
		lib_p=int(sl[2:])
		lib_s=sp_inst_funcs[lib_p]['s']
		lib_l=sp_inst_funcs[lib_p]['l']
		lib_len=sp_inst_funcs[lib_p]['len']
		...
		if target!=[]:
			mark|=(1<<20)
		string.append('store i32 '+str(mark+node_id)+', i32* @dfi_reg_signal, align 4\n')
		now_id=base_id
		if source!=[]:
			string.append('%'+str(now_id)+' = ptrtoint '+source+' to i32\n')
			string.append('store i32 %'+str(now_id)+', i32* @dfi_reg_signal, align 4\n')#load_from is the ptr
			now_id+=1
		if target!=[]:
			string.append('%'+str(now_id)+' = ptrtoint '+target+' to i32\n')
			string.append('store i32 %'+str(now_id)+', i32* @dfi_reg_signal, align 4\n')#load_from is the ptr
			now_id+=1
		if(rlen<=32):
			string.append('store '+load_from[lib_len]+', i32* @dfi_reg_signal, align 4\n')
		else:
			string.append('%'+str(now_id)+' = trunc '+load_from[lib_len]+' to i32\n')
			string.append('%'+str(now_id+1)+' = lshr '+load_from[lib_len]+', 32\n')
			string.append('%'+str(now_id+2)+' = trunc i'+str(rlen)+' %'+str(now_id+1)+' to i32\n')
			string.append('store i32 %'+str(now_id)+', i32* @dfi_reg_signal, align 4\n')
			string.append('store i32 %'+str(now_id+2)+', i32* @dfi_reg_signal, align 4\n')
		if(signal_delay):
			string.append('call void @'+sfunc['dfi_delay']+'(i32 '+str(signal_delay)+')\n')
	elif sl=='bt':
		string.append('store i32 '+str((1<<21)|(1<<16)|ret_id)+', i32* @dfi_reg_signal, align 4\n')
		string.append('store i32 %'+str(init_base_id+1)+', i32* @dfi_reg_signal, align 4\n')	
		if(signal_delay):
			string.append('call void @'+sfunc['dfi_delay']+'(i32 '+str(signal_delay)+')\n')
	else:
		string=[]
...
def inst_sl(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos):
	if exp_mode=='noinstr':
		...
		return inst_sl_cfi(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos)
	elif exp_mode=='core2':
		return inst_sl_new_5(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos)
	elif exp_mode=='soft':
		return inst_sl_old_4(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos)
	elif exp_mode=='softtest':
		return inst_sl_old_cycletest(fake,base_id,init_base_id,load_from,node_id,sl,rds,pos)
	elif exp_mode=='ori':
...
def inst_initialize_func(text,mode):
	...
	if mode=='roccinstr':
		if new_dfi==0:
			...
			text.insert(linemain+1,'call void @'+sfunc['dfi_new_init']+'()\n')
	elif mode=='core2':
		text.insert(linemain+1,'call void @'+sfunc['init_2core_dfi']+'()\n')
	elif mode=='soft':
		...
		text.insert(linemain+1,'call void @'+sfunc['cfi_init']+'()\n')
...
def remove_initialize_func(text,mode):
	...
	if mode=='roccinstr':
		...
		rexp='call void @'+sfunc['init_2core_dfi']+'()\n'
	while i<len(text):
		if text[i]==rexp:
			del text[i]
			return
...
def inst_ll(idinst,ll_name,ll_out_name):
	...
	usr_func = [] #record the usr function(has the code)
	...
	"""
	The code below is to find the usr functions
	"""
	...
	while i_i<len(idinst) and i_t<len(text):
		if idinst[i_i]['type'] in ['s','l','bc','bt']:
			record_type=idinst[i_i]['type']
			if idinst[i_i]['type']=='bc' and exp_mode!='cfi':
				sp_inst_func=check_if_special_inst_func(idinst[i_i]['inst'])
				if sp_inst_func==-1:
					i_i+=1
					continue
			...
			if i_i!=len(idinst)-1 and inst_cmp_rd_ll(idinst[i_i]['type'],idinst[i_i]['inst'],text[i_t]):
				if idinst[i_i]['type'] == 'bc':
					expression = '@(.*?)[(]'
					inst_str = str(idinst[i_i]['inst'])
					func_nametmp = re.findall(expression,inst_str)
					#print func_nametmp
				#print record_type,'|',idinst[i_i]['type'],'|',idinst[i_i]['inst'],'|',text[i_t]
				r=inst_match_target(record_type,text[i_t])
				#print i_i,len(idinst),idinst[i_i]['inst'],text[i_t],r
				if(idinst[i_i]['del']):
					invalid.append(len(inst_records))
				if idinst[i_i]['type'] in ['bc','bt']:#if we instrument bc and bt in front of them, we should set the "id to this instr" the old_id
					if idinst[i_i]['type']=='bt' or (sp_inst_funcs[sp_inst_func]['pos']==0 and exp_mode != 'cfi' ):
						inst_records.append([i_t,record_type,idinst[i_i]['id'],old_id,r[0]])
					else:
						if exp_mode == 'cfi':
							if len(func_nametmp)!=0:
								inst_records.append([i_t,record_type,idinst[i_i]['id'],now_id,func_nametmp[0]])
								#print [i_t,record_type,idinst[i_i]['id'],now_id,func_nametmp[0]]
						else:
							inst_records.append([i_t,record_type,idinst[i_i]['id'],now_id,r[0]])
				else:
					inst_records.append([i_t,record_type,idinst[i_i]['id'],now_id,r[0]])
	...
	if i_i<len(idinst)-1:
		print "ERROR, .rd not match .ll",exp_mode,idinst[i_i],i_i,len(idinst),i_t
		exit(1)
	...
	for ir in inst_records:
		if ir[1]=='f':
			...
			#text.insert(ir[0]+1,'\n')
		elif (ir[1]=='s' or ir[1]=='l') and exp_mode!='cfi':#instrument after them
			...
			#text.insert(ir[0]+1,'\n')
		elif ((len(ir[1])>2 and ir[1][0:2]=='bc') or ir[1]=='bt') and exp_mode!='cfi':#instrument before them
			if ir[1]=='bt' or sp_inst_funcs[int(ir[1][2:])]['pos']==0:
				...
				for t in add:
					text.insert(ir[0]+1,t)
				text.insert(ir[0]+1,'>>>>>S>>>>>'+ir[1]+'\n')
		elif len(ir[1])>2 and ir[1][0:2]=='bc' and exp_mode=='cfi':#instrument before them
			#ir[4] is func name
			if ir[4] not in usr_func and type(ir[4])==str:
				print '======>lib func cfi'
				print ir[4]
				add=['call void asm sideeffect ".word '+hex(0x0400000b)+'", ""()\n']
				add.reverse()
				text.insert(ir[0],'<<<<<E<<<<<'+ir[1]+'\n')
				for t in add:
					text.insert(ir[0],t)
				...
				add=['call void asm sideeffect ".word '+hex(0x0200000b)+'", ""()\n']
				add.reverse
				text.insert(ir[0]+4,'<<<<<E<<<<<'+ir[1]+'\n')
				for t in add:
					text.insert(ir[0]+4,t)
				text.insert(ir[0]+4,'>>>>>S>>>>>'+ir[1]+'\n')
		i+=1
	...
	if exp_mode=='core2' or exp_mode=='soft' or exp_mode=='softtest' or exp_mode=='roccinstr' or exp_mode=='ori' or exp_mode=='cfi':
	...
	if debugfunc == 1:
		...
		cfi_inst_rocc_debug(text)
	elif debugfunc == 4:
		dfi_inst_rocc_debug(text)
...
def inst_write_rds_roccinstr(idinst):
	...
	res_addr=0
	for i in range(n_addr):
		data[i][0]=res_addr|(len(data[i][1])<<32)|(1<<63)      #wxrqw
		res_addr=res_addr+len(data[i][1])
	mem_hword=[]
	mem_hword.append((now_addr)&0xffff)
	mem_hword.append((now_addr>>16)&0xffff)
	mem_hword.append((now_addr>>32)&0xffff)
	mem_hword.append((now_addr>>48)&0xffff)
	for d in data:
	...
	table_size = len(mem_hword)*2 - 8
	res_hword=[]
	res_hword.append((table_size)&0xffff)
	res_hword.append((table_size>>16)&0xffff)
	res_hword.append((table_size>>32)&0xffff)
	res_hword.append((table_size>>48)&0xffff)
	for d in mem_hword:
		res_hword.append(d)
...
def revise_rocc_lib_instr(text):
	...
	while i<len(text):
		if state==0:
			if len(re.findall("call\s+\S*dfi_rocc_debug\S*",text[i]))>0 or len(re.findall("call\s+\S*dfi_new_rocc_debug\S*",text[i]))>0:
				temp[0]=i
				state=1
		elif state==1:
	...
	while i<len(text):
		if state==0:
			...
			if len(r)>0:
				data=int(r[0])
				temp[4]=data
				if (temp[4]>>30)==2 and ((temp[4]>>27)&0x7)>0:
					state=2
				else:
					state=0
			else:
			...
			if len(re.findall("^\s*call\s+\S+",text[i])):
				temp[2]=i
				if (temp[4]>>30)==2 and ((temp[4]>>27)&0x3)>0:
					temp[3]=1
					matched.append(copy.deepcopy(temp))
				else:
					temp[3]=2
...
for i in p:
	if sys.argv[i]=='-init':
	...
	elif sys.argv[i]=='-usr_instr':
		is_usr_instr=1
		usr_instr_file=sys.argv[i+1]
		...
		hdfi_mode=1
	elif sys.argv[i]=='-hdfi_instr':
		hdfi_instr=1
	elif sys.argv[i]=='-whlist':
		whlist_instr=1
	...
	elif sys.argv[i]=='-debugcfi':
		debugfunc=3
		print 'debug cfi func is inserted'
	elif sys.argv[i]=='-debugdfi':
		debugfunc=4
		print 'debug dfi func is inserted'
...
if sys.argv[1]=='test' or (sys.argv[1] == 'ripe_attack_generateror' and exp_mode=='cfi'):
	insert_debug_rules=['^\s*ret',]
else:
	insert_debug_rules=[
	'^\s*ret',
	'call void @mylongjmp\(',
...
if exp_mode=='noinstr' and mode==0:
...
elif (exp_mode=='noinstr' and mode==2) or exp_mode=='core2' or exp_mode=='roccinstr' or exp_mode=='soft' or exp_mode=='softtest' or exp_mode=='ori' or exp_mode=='cfi':
	...
	#test spec-->opt on. test hearbleed off
	# if hdfi_mode==0 and hdfi_instr==0 and whlist_instr==0:
	# 	idinst_opt_rename(idinst)
	# 	print 'Rename optimization complete'
	...
	if exp_mode=='noinstr':
		text_noinstr=read_file(sys.argv[1]+'.ll')
		cfi_inst_rocc_debug(text_noinstr)
		f=open(sys.argv[1]+'.ll','wb')
		for t in text_noinstr:
			f.write(t)
		f.close()
